...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// If all the pids are being monitored
	if (table[reg.ax].monitored == 2) {
		spin_lock(&pidlist_lock);
		// Log a message if the pid is not in the blacklist
		if (check_pid_monitored(reg.ax, pid) == 0) {
			log_message(pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
		spin_unlock(&pidlist_lock);
	} else {
		spin_lock(&pidlist_lock);
		// Otherwise, log a message if the pid is in the whitelist
		if (check_pid_monitored(reg.ax, pid) == 1) {
			log_message(pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
		spin_unlock(&pidlist_lock);
	}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// EINVAL if the syscall is less than 0, the same as the custom syscall,
	// or a number greater than the total number of syscalls
	if (syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls) {
		return -EINVAL;
	}

	// EIVNAL if the pid is less than 0, or an existing pid
	if (pid < 0 || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && pid != 0)) {
		return -EINVAL;
	}

	// EPERM if the first two commands and not in root
	...
	// EPERM if the last two commands and not in root AND the pid doesn't have
	...
	// EINVAL if attempting to release or start/stop monitoring a call that
	// isn't being intercepted yet
	...
	switch(cmd) {
		...
		// EINVAL if the the pid is not being monitored by the syscall
		...
		// EBUSY if the pid is already being monitored
		case REQUEST_START_MONITORING:
			...
			if (table[syscall].monitored != 2 && check_pid_monitored(syscall, pid) == 1) {
				...
				if (pid == 0 && table[syscall].listcount == 0) {
					...
					spin_unlock(&calltable_lock);
					return -EBUSY;
				} else if (check_pid_monitored(syscall, pid) == 0 && pid != 0) {
					spin_unlock(&pidlist_lock);
					spin_unlock(&calltable_lock);
					return -EBUSY;
				}
		...
		// EBUSY if the syscall has already been intercepted
	...
	switch(cmd) {
		...
		// Store the original syscall in table and intercept the
		// original syscall
		case REQUEST_SYSCALL_INTERCEPT:
			spin_lock(&calltable_lock);
		...
		// Restore the original syscall to the sys_call_table
      ...
      set_addr_rw((unsigned long) sys_call_table);
		...
		// Start monitoring the pid for the syscall
		case REQUEST_START_MONITORING:
			spin_lock(&calltable_lock);

			// If pid is 0, monitor all the pids and destroy the whitelist
			if (pid == 0) {
				spin_lock(&pidlist_lock);
			...
			// If all pids are being monitored, delete the pid from the blacklist
			} else if (table[syscall].monitored == 2) {
				spin_lock(&pidlist_lock);
				del_pid_sysc(pid, syscall);
				spin_unlock(&pidlist_lock);

			// Otherwise, add the pid to the whitelist
			} else {
				spin_lock(&pidlist_lock);
		...
		// Stop monitoring the pid for the syscall
		case REQUEST_STOP_MONITORING:

			// If pid is 0, stop monitoring all the pids and destroy the blacklist
			if (pid == 0) {
				spin_lock(&calltable_lock);
				spin_lock(&pidlist_lock);
				destroy_list(syscall);
				...
				// If all the pids are being monitored, add the pid to the blacklist
				...
				//otherwise, remove the pid from the whitelist
				} else {
					spin_lock(&calltable_lock);
					spin_lock(&pidlist_lock);
					del_pid_sysc(pid, syscall);
...
static int init_function(void) {
	...
	//Save the original MY_CUSTOM_SYSCALL and hijack it with my_sys_call
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;

 	//Save the original __NR_exit_group and hijack it
 	orig_exit_group = sys_call_table[__NR_exit_group];
	sys_call_table[__NR_exit_group] = my_exit_group;

 	set_addr_ro((unsigned long) sys_call_table);

	//Initializing the pid lists and the table entries for each syscall
	for (; s < NR_syscalls; s++) {
...
static void exit_function(void)
{
	...
	//Restore original syscalls and the original exit group
	...
	//Restore all original system calls if they have been intercepted
	//but not released
	for (; i < NR_syscalls; i++) {
		spin_lock(&calltable_lock);
